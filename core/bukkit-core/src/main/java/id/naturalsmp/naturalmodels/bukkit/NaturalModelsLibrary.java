...
public final class NaturalModelsLibrary {
    ...
    private static @NotNull LibraryData register(@NotNull String group, @NotNull String artifact,
            @NotNull Function<LibraryData.Builder, LibraryData.Builder> function) {
    ...
    public record LibraryData(
        ...
        private static class Builder {
            ...
            @NotNull
            @Unmodifiable
            Set<String> subModules = Collections.emptySet();
            @NotNull
            ...
            @NotNull
            Builder subModules(@NotNull String... subModules) {
        ...
        private @NotNull Stream<Library> toLibby(@NotNull AbstractNaturalModelsPlugin plugin) {
            ...
            return Stream.concat(
                    Stream.of(artifact),
                    subModules.stream()).map(name -> {
                        var libs = Library.builder()
                                .groupId(group)
                                .artifactId(name)
                                .version(version);
                        if (relocation != null)
                            libs.relocate(new Relocation(relocation, "kr{}toxicity{}model{}shaded{}" + relocation));
                        return libs.build();
                    });
        }
    }
